...
const registerSchema = z.object({
    ...
    terms_accepted: z.literal("on", { message: "You must accept the terms of service." }),
});
...
export async function registerUser(c) {
    ...
    try {
        ...
        await client.queryArray(
            `INSERT INTO zephyr_users (username, password_hash, role, birthdate, terms_accepted) 
             VALUES ($1, $2, $3, $4, TRUE)`,
        ...
        if (error instanceof z.ZodError) {
            ...
            return new Response(`Validation Error: ${error.errors.map((e) => e.message).join(", ")}`, {
                status: 400,
            });
        }
...
// Handle fetching account information
export async function getAccountInfo(username) {
    try {
        // Fetch account details from the database
        const result = await client.queryObject(
            `SELECT username, role, terms_accepted, created_at 
             FROM zephyr_users 
             WHERE username = $1`,
            [username]
        );
